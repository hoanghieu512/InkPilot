...
function main(): void {
  ...
  const row = db.prepare(`
    SELECT
      COUNT(*) as total,
      SUM(CASE WHEN score >= 8   THEN 1 ELSE 0 END) as hot,
      SUM(CASE WHEN score >= 6 AND score < 8 THEN 1 ELSE 0 END) as other,
      SUM(CASE WHEN score < 6    THEN 1 ELSE 0 END) as dismissed,
      ...
      SUM(CASE WHEN score >= 7  AND score < 8  THEN 1 ELSE 0 END) as b7_8,
  ...
  console.log(`  HOT (8+):      ${row.hot}  (${pct(row.hot, row.total)})`);
  console.log(`  OTHER (6–7.9): ${row.other}  (${pct(row.other, row.total)})`);
  ...
  const buckets: Array<{ label: string; count: number }> = [
    ...
    { label: '7–8',  count: row.b7_8 },
    { label: '8–9',  count: row.b8_9 },
    { label: '9–10', count: row.b9_10 },
  ];

  for (const b of buckets) {
    const isHot = b.label === '8–9' || b.label === '9–10';
    const isNearHot = b.label === '7–8';
    const tag = isHot ? ' ← HOT' : isNearHot ? ' ← near-HOT' : '';
    console.log(`  ${b.label.padEnd(6)}  ${String(b.count).padStart(5)}  ${bar(b.count, row.total)}${tag}`);
  }
