...
npm run list                     # tiered inbox: HOT (8+) then OTHER (6–7.9)
npm run list -- --hot            # HOT only
npm run list -- --other          # OTHER only (6–7.9)
...
| `src/config/index.ts` | `AI_MODELS` + `.env` loader |
...
**6/11 sources enabled.** The Block and Base Blog disabled — both behind Cloudflare (all UA variants return 403, no alternate endpoints available). Coin68, Tạp Chí Bitcoin, MarkTechPost also disabled (XML errors / timeouts).
